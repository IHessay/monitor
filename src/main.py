...
from src.sogou import SogouBlockedError, create_session, fetch_account_articles, resolve_sogou_link
...
def main() -> int:
    ...
    accounts = config.get("monitor", {}).get("accounts", [])
    max_articles = config.get("monitor", {}).get("max_articles_per_account", 10)
    send_empty = config.get("monitor", {}).get("send_empty_notice", False)
    filter_config = config.get("filter", {})

    if not accounts:
        logger.warning("No accounts configured. Please run discover.yml and update config/monitor.yaml")
        return 0

    state = State.load()
    state.set_monitored_accounts(accounts)
    ...
    for idx, account in enumerate(accounts):
        if idx > 0:
            # Be polite: wait between accounts.
            time.sleep(random.uniform(2, 5))

        logger.info("Processing account: %s", account)
        try:
            articles = fetch_account_articles(session, account, max_articles=max_articles)
        except SogouBlockedError:
            logger.error("Sogou blocked for account=%s; stopping this account", account)
            continue
        except Exception as exc:
            logger.exception("Unexpected error fetching account=%s: %s", account, exc)
            continue
        ...
        for art in articles:
            ...
            if state.is_seen(art_id):
                logger.debug("Already seen: %s", title)
                continue
