...
def test_match_year_and_job(filter_config):
    # Must also contain a law keyword.
    assert check_match("2027届法学秋招启动", "", filter_config) is not None


def test_year_and_job_without_law_excluded(filter_config):
    assert check_match("2027届校招启动", "", filter_config) is None
...
def test_non_law_recruitment_excluded(filter_config):
    assert check_match("某互联网公司2027校园招聘", "", filter_config) is None
